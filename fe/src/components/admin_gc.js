...
async function loadGameContents(filters = {}) {
    try {
        ...
        if ($('total-game-contents')) {
            $('total-game-contents').textContent =
                data.data?.total || currentGameContents.length;
        }
    } catch (err) {
...
function renderGameContentsTable(contents) {
    ...
    tbody.innerHTML = contents.map(content => `
        <tr>
            <td title="${content.content_id}">
                ${content.content_id.substring(0, 8)}...
            </td>
            <td title="${content.game_id}">
                ${content.game_id.substring(0, 8)}...
            </td>
            <td>
                <span class="badge badge-info">Level ${content.level}</span>
            </td>
            <td>
                <span class="badge badge-secondary">${content.content_type}</span>
            </td>
            <td
                style="max-width: 200px; overflow: hidden; text-overflow: ellipsis;"
                title="${content.question_text}"
            >
                ${content.question_text.substring(0, 50)}
                ${content.question_text.length > 50 ? '...' : ''}
            </td>
            <td>
                ${content.emotion
            ...
            <td class="actions">
                <button
                    ...
                    onclick="window.editGameContent('${content.content_id}')"
                    title="Chỉnh sửa"
                >
                    ✏️
                </button>
                <button
                    ...
                    onclick="window.deleteGameContent('${content.content_id}')"
                    title="Xóa"
                >
                    🗑️
                </button>
        ...
        try {
            ...
            if (currentMediaFile) {
                ...
                const uploadRes = await fetch(
                    `${API_URL}/game-contents/upload-media`,
                    {
                        method: 'POST',
                        body: formData
                    }
            ...
            if (editingGameContentId) {
                res = await fetchAPI(
                    `${API_URL}/game-contents/${editingGameContentId}`,
                    {
                        method: "PUT",
...
window.editGameContent = async content_id => {
    ...
    try {
        ...
        $('game-content-modal-title').textContent =
            '✏️ Chỉnh sửa Nội dung Game';
