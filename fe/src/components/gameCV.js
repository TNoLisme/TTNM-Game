...
const CV_GAME_CONFIG = {
  GV1: {
    ...
    emptyLevelMessage: (level) =>
      `Không có tình huống nào ở level ${level}. Vui lòng chọn level khác.`,
    introBuilder: (scenario) =>
      `Con nghe tình huống nhé. ${scenario.description}`,
    summaryBuilder: (emotion) =>
      `Hôm nay con đã thể hiện cảm xúc rất tốt! Cảm xúc con làm giỏi nhất là ${
        emotion || "tất cả"
      }. Lần sau mình luyện thêm nhé!`,
  },
  GV2: {
    id: "GV2",
    ...
    emptyEmotionMessage: (emotion) =>
      `Hiện chưa có bài luyện cho cảm xúc "${emotion}". Vui lòng quay lại chọn cảm xúc khác.`,
    emptyLevelMessage: (level) =>
      `Không có yêu cầu nào ở level ${level}. Vui lòng chọn level khác.`,
    introBuilder: (scenario) =>
      `Hãy thể hiện khuôn mặt ${scenario.target_emotion} nhé. ${scenario.description}`,
    summaryBuilder: (emotion) =>
      `Con đã luyện tập các biểu cảm ${
        emotion || "đa dạng"
      } rất tốt hôm nay! Nhớ giữ phong độ nhé!`,
  },
};
...
const CV_LEARNING_CARDS = {
  vui: {
    title: "Vui",
    description:
      "Lan được tặng một món quà bất ngờ nên Lan rất vui và mỉm cười.",
  },
  buồn: {
    title: "Buồn",
  ...
  "ngạc nhiên": {
    title: "Ngạc nhiên",
    description:
      "Huy mở hộp quà ra và thấy món đồ chơi mình rất thích nên rất ngạc nhiên.",
  },
  "ghê tởm": {
    title: "Ghê tởm",
...
async function speakText(text, voice = "thuminh", speed = 0) {
...
function startRoundTimer() {
  ...
  const tick = () => {
    ...
    timerEl.classList.toggle(
      "is-warning",
      remainingMs <= ROUND_TIMER_WARNING_MS
    );
  };
...
async function initGame() {
  ...
  if (!userId) {
    ...
    showError(
      "Vui lòng đăng nhập để chơi game. Đang chuyển về trang đăng nhập..."
    );
  ...
  if (gameState.config?.requiresEmotion) {
    ...
    gameState.selectedEmotion =
      decodeURIComponent(selectedEmotion).toLowerCase();
  ...
  if (gameState.config?.requiresEmotion && gameState.selectedEmotion) {
    ...
    gameState.scenarios = gameState.scenarios.filter(
      (scenario) => (scenario.target_emotion || "").toLowerCase() === emotionKey
    );
    console.log(
  ...
  if (gameState.scenarios.length === 0) {
    ...
    if (gameState.config?.requiresEmotion && gameState.selectedEmotion) {
      emptyMessage =
        typeof gameState.config?.emptyEmotionMessage === "function"
      ...
      emptyMessage = gameState.config.emptyLevelMessage(
...
async function loadFaceApiModels() {
  try {
    ...
    showError(
      "Không thể tải mô hình nhận diện. Vui lòng tải models từ https://github.com/justadudewhohacks/face-api.js/tree/master/weights và đặt vào thư mục /models"
    );
  }
}
...
async function loadScenarios(level = 1) {
  try {
    ...
    console.log(
      `Scenarios loaded for level ${level}:`,
...
function startScenario(index) {
  ...
  const introSpeech =
    typeof gameState.config?.introBuilder === "function"
  ...
  if (scenarioImage && gameState.currentScenario.image_path) {
    ...
    scenarioImage.onerror = () => {
      ...
      if (imageContainer) {
        ...
        imageContainer.innerHTML = `<div style="color: #666; font-size: 1.2rem; text-align: center; padding: 20px;">${
          EMOTION_ICONS[gameState.targetEmotion] || "📷"
...
function showHint() {
  ...
  hintContainer.innerHTML = `
        <div class="hint-content">
            <div class="emotion-animation ${emotion}">${
...
async function startDetection() {
  ...
  try {
    ...
    // Speak instruction
    const instruction =
      gameState.currentScenario.instruction ||
    ...
    if (
      ...
      errorMessage +=
        "Vui lòng cho phép quyền truy cập camera trong cài đặt trình duyệt.";
    } else if (
...
async function startSession() {
  try {
    ...
    console.log(
      `📤 Sending start_session request: user_id=${userId}, game_type=${gameType}`
    );
      ...
      if (detections.length > 0) {
        ...
        if (gameEmotion === gameState.targetEmotion && confidence >= 0.6) {
          ...
          if (confidence > gameState.bestConfidence) {
            ...
            console.log(
              `🎯 New best confidence for ${gameEmotion}: ${(
                confidence * 100
              ).toFixed(1)}% (>= 60%)`
            );
          }
...
function registerCvIncorrectAndMaybeShowLearn() {
  ...
  if (
    cvEmotionErrorCounts[key] >= CV_MAX_INCORRECT_BEFORE_LEARN &&
    !cvLearnedEmotions.has(key)
  ) {
...
function showCvLearningCard(emotionKey) {
  ...
  const card = CV_LEARNING_CARDS[emotionKey] || {
    title: emotionKey,
    description: "",
  };
  ...
  if (learnBtn) {
    ...
    learnBtn.onclick = () => {
      window.location.href = `/src/pages/learn.html?emotion=${encodeURIComponent(
        learnEmotion
      )}`;
    };
  }
...
async function handleSuccess() {
  ...
  console.log(
    `💾 Saving success with best confidence: ${gameState.bestConfidence} (${bestConfidencePercent}%)`
  );
...
async function handleTimeout() {
  ...
  const nextIndex = gameState.currentScenarioIndex + 1;
  const isLastTurn =
    nextIndex >= (gameState.scenarios ? gameState.scenarios.length : 0);
  if (!(gameState.gameId !== "GV1" && isLastTurn)) {
    speakText(
      "Chúng ta thử lại thêm lần nữa nhé! Lần sau mình sẽ làm tốt hơn!"
    );
  }
...
function showSuccessAnimation() {
  ...
  successEl.innerHTML = `
        <div class="success-content">
            ...
            <div class="success-message">Con đã thể hiện ${
              gameState.targetEmotion
            } rất tốt!</div>
...
async function saveResult(success, confidenceScore = 0.0) {
  try {
    ...
    console.log(
      `💾 Saving result: success=${success}, confidence_score=${confidenceScore}% (thang 100)`
    );
...
async function endSession() {
  try {
    ...
    if (data.emotion_errors) {
      try {
        const emotionErrors =
          typeof data.emotion_errors === "string"
        ...
        if (gameState.gameId === "GV2") {
          ...
          if (
            emotionErrors[targetEmotion] &&
            emotionErrors[targetEmotion].best_confidence
          ) {
            emotionKey = targetEmotion;
          } else if (
            emotionErrors[targetEmotionLower] &&
            emotionErrors[targetEmotionLower].best_confidence
          ) {
            ...
            for (const key of possibleKeys) {
              ...
              if (
                targetWords.some((word) => keyWords.includes(word)) ||
          ...
          if (
            ...
            emotionErrors[emotionKey].best_confidence
          ) {
            bestConfidenceScore = Math.round(
              emotionErrors[emotionKey].best_confidence
            );
            gameState.finalBestEmotion = emotionKey;
          }
          console.log(
            `📊 Best confidence score for ${targetEmotion} (found as "${emotionKey}"): ${bestConfidenceScore}%`
          );
        } else {
    ...
    try {
      ...
      const existingSessions = JSON.parse(
        localStorage.getItem("gameCV_sessions") || "[]"
      );
...
async function speakWithFPTAI(text, requestId) {
  try {
    ...
    if (audioUrl) {
      ...
      const proxyUrl = `${API_URL}/games/cv/audio-proxy?url=${encodeURIComponent(
        audioUrl
      )}`;
      if (typeof requestId === "number" && requestId !== ttsRequestId) {
        return false;
...
function showSummary() {
  ...
  const summaryText =
    typeof gameState.config?.summaryBuilder === "function"
...
function showGameCompleteModal() {
  ...
  if (!modal) {
    ...
    if (window.egModal && typeof window.egModal.confirm === "function") {
      window.egModal
        .confirm(
          "Game đã hoàn thành! Con muốn chơi tiếp hay nghỉ một lát?",
          "Hoàn thành",
          "Chơi tiếp",
          "Về trang chủ"
        )
      ...
      handleChoice(
        confirm("Game đã hoàn thành! Con muốn chơi tiếp hay nghỉ một lát?")
      );
    }
    return;
  }
  ...
  if (completionMessage) {
    if (gameState.gameId === "GV1") {
      ...
      completionMessage.textContent =
        "Con đã hoàn thành thử thách! Làm tốt lắm!";
    }
  }
  ...
  if (scoreDisplay) {
    if (gameState.gameId === "GV1") {
      ...
      const total = Number(
        (gameState.scenarios && gameState.scenarios.length) || 0
      );
...
function saveGameProgress() {
  try {
    const progressKey = `gameCV_progress_${gameState.gameId}_${
      gameState.selectedLevel || 1
    }`;
    const progress = {
    ...
    console.log(
      `💾 Saved game progress: scenario ${gameState.currentScenarioIndex + 1}/${
        gameState.scenarios.length
      }`
    );
  } catch (e) {
...
function showContinueGameModal(progress, progressKey) {
  return new Promise((resolve) => {
    ...
    modal.innerHTML = `
            ...
            ">
                ...
                <p style="margin: 8px 0 20px; font-size: 15px; line-height: 1.5;">
                    Bạn đang chơi dở level <strong style=\"color:#ffd700;\">${
                      gameState.selectedLevel
                    }</strong>.<br>
                    Tiếp tục từ màn <strong style=\"color:#ffd700;\">${
                      progress.currentScenarioIndex + 1
                    }</strong> nhé?
                </p>
    ...
    document.getElementById("continue-yes-btn").onclick = () => {
      ...
      console.log(
        `✅ Continuing from saved progress: scenario ${
    ...
    document.getElementById("continue-no-btn").onclick = () => {
      cleanup();
      console.log(
        `🔄 User chose to start from beginning - ending old session ${progress.sessionId}`
      );
      try {
        const blob = new Blob(
          [JSON.stringify({ session_id: progress.sessionId })],
          {
    ...
    if (savedProgress) {
      ...
      if (
        ...
        if (isReload) {
          ...
          console.log(
            "🔄 Reload detected - auto continuing from saved progress"
          );
          gameState.sessionId = progress.sessionId;
          gameState.currentScenarioIndex = progress.currentScenarioIndex || 0;
    ...
    if ("speechSynthesis" in window) {
      ...
      window.speechSynthesis.onvoiceschanged = () => {
        ...
        if (hasVietnameseVoice) {
          ...
          if (isDevelopment) {
            console.log(
              "No Vietnamese voice found, will use FPT AI TTS for Vietnamese"
            );
          }
        }
      };
